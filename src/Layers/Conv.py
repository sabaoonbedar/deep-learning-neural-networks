...
class Conv(BaseLayer):
    ...
    def __init__(self, stride_shape, conv_shape, num_kernels):
        super().__init__()
        self.trainable = True
        self.testing_phase = False  # Included for completeness
    ...
    def calculate_regularization_loss(self):
        """
        Return the regularization penalty for this layer, if the optimizer provides one.
        """
        if self._weight_optimizer is not None and self._weight_optimizer.regularizer is not None:
            return self._weight_optimizer.regularizer.norm(self.weights)
        return 0
